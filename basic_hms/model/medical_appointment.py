...
class medical_appointment(models.Model):
    ...
    def _get_doctor_duration(self):
        for each in self:
            if each.doctor_id:
                each.duration = each.doctor_id.duration
            else:
                each.duration = 0

    @api.onchange('doctor_id')
    def _get_doc_duration(self):
        if self.doctor_id:
            self.duration = self.doctor_id.duration

    @api.depends('duration', 'appointment_date')
    def _get_appointment_end(self):
        for rec in self:
            rec.appointment_date + timedelta(hours=rec.duration)

    @api.onchange('duration', 'appointment_date')
    def _get_app_end(self):
        if self.duration and self.appointment_date:
            self.appointment_end = self.appointment_date + timedelta(hours=self.duration)
    ...
    appointment_end = fields.Datetime('Appointment End', required=True, default=_get_appointment_end)
    ...
    duration = fields.Float('Duration', default=_get_doctor_duration)
